...
test('it renders a view with a model', function(assert) {
  ...
  assert.equal(this.$('input[type="name"]').length, 1, 'should be only 1 input name field - email');

  // Test if proper values are placed into form
  assert.equal(this.$('input[type="name"]').val(), this.get('model.email'), 'names should match');
